...
const modal = {
  ...
  select(message, options = [], title = '请选择', confirmText = '确定', cancelText = '取消') {
    return new Promise((resolve) => {
      const container = createModalContainer();
      ...
      const optionsHtml = options.map((opt, index) => `
        <div class="modal-option" data-index="${index}" style="
          padding: 12px 16px;
          margin: 6px 0;
      ...
      modal.innerHTML = `
        <h3 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600; color: #333; text-align: center;">${title}</h3>
        <div style="color: #666; font-size: 14px; line-height: 1.6; text-align: center; margin-bottom: 16px;">${message}</div>
        <div class="modal-options" style="max-height: 300px; overflow-y: auto;">
          ${optionsHtml}
        </div>
        <div style="display: flex; gap: 12px; margin-top: 20px;">
          <button class="modal-cancel-btn" style="flex: 1; padding: 10px 20px; border: 1px solid #d9d9d9; border-radius: 6px; background: white; color: #666; font-size: 14px; cursor: pointer; transition: all 0.3s;">${cancelText}</button>
          <button class="modal-confirm-btn" style="flex: 1; padding: 10px 20px; border: none; border-radius: 6px; background: #1890ff; color: white; font-size: 14px; cursor: pointer; transition: all 0.3s; font-weight: 600;">${confirmText}</button>
        </div>
      `;
      ...
      optionElements.forEach((optEl, index) => {
        optEl.addEventListener('click', () => {
          ...
          optEl.style.borderColor = '#1890ff';
          optEl.style.background = '#e6f7ff';
          optEl.style.color = '#1890ff';
        });

        optEl.addEventListener('mouseenter', () => {
          if (selectedValue !== options[index]) {
            optEl.style.borderColor = '#1890ff';
            optEl.style.background = '#f0f5ff';
          }
        });

        optEl.addEventListener('mouseleave', () => {
          if (selectedValue !== options[index]) {
            optEl.style.borderColor = '#e0e0e0';
            optEl.style.background = 'white';
            optEl.style.color = '#333';
          }
        });
      });
      ...
      confirmBtn.addEventListener('click', () => {
        if (!selectedValue) {
          ...
          confirmBtn.textContent = '请先选择一个选项';
