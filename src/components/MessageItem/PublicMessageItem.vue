...
const renderer = new marked.Renderer();
  renderer.code = function({ text, lang }) {
    ...
    const copyCode = `navigator.clipboard.writeText(decodeURIComponent('${encodedCode}')).then(() => { const notice = this.closest('.highlight-tools').querySelector('.copy-notice'); if(notice) { notice.textContent = '已复制'; notice.style.color = '#4CAF50'; setTimeout(() => { notice.textContent = ''; }, 2000); }}).catch(err => console.error('复制失败:', err));`;
  ...
  const loadMoreAndSearch = async () => {
    ...
    await new Promise((resolve) => {
      ...
      if (chatType === 'public') {
        ...
        const privateMsgs = chatStore.privateMessages[privateUserId] || [];
        const friendId = privateUserId.startsWith('private_') ? privateUserId.replace('private_', '') : privateUserId;
