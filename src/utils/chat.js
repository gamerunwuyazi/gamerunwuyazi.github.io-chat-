...
let currentSendChatType = sessionStore.currentSendChatType; // 当前要发送的聊天类型（main或group）
let selectedGroupIdForCard = sessionStore.selectedGroupIdForCard; // 当前选中的要发送的群组ID
let currentSharedGroup = null; // 当前要分享的群组信息
let chatHistory = null;
let groupsList = []; // 群组列表数据
...
export function setCurrentSessionToken(token) {
    ...
    window.currentSessionToken = token; // 更新全局变量
    if (token) {
...
export function initializeChat() {
        ...
        try {
            ...
            if (typeof initializeChatInternal === 'function') {
                ...
                // 如果内部函数不存在，执行基本初始化
                console.warn('内部初始化函数不存在，执行基本初始化');
            ...
            // 私信会话状态仍然在initializeChat中应用
            setTimeout(() => {
                // 检查是否有保存的私信会话
                if (currentPrivateChatUserId) {
                    console.log('自动应用保存的私信会话状态:', {
...
function moveFriendToTop(friendId) {
    ...
    const friendItems = friendsListElement.querySelectorAll('.friend-item');
    for (const item of friendItems) {
        if (String(item.getAttribute('data-user-id')) === String(friendId)) {
            // 将好友移到列表顶部
            friendsListElement.insertBefore(item, friendsListElement.firstChild);
            
            // 更新内存中的好友列表顺序
            const friendIndex = friendsList.findIndex(f => String(f.id) === String(friendId));
            if (friendIndex > 0) {
                const friend = friendsList.splice(friendIndex, 1)[0];
...
function displayUserInfo(user) {
    ...
    // 更新用户头像
    if (avatarUrl && !isSvgAvatar) {
        let fullAvatarUrl = avatarUrl.startsWith('http://') || avatarUrl.startsWith('https://') 
            ? avatarUrl 
    ...
    // 显示个性签名
    const signatureSection = document.querySelector('#userProfileModal .signature-section');
    const modalUserSignature = document.getElementById('modalUserSignature');
    if (signatureSection && modalUserSignature) {
        const signature = user.signature || '';
        if (signature && signature.trim()) {
            modalUserSignature.textContent = unescapeHtml(signature);
...
function initializeChatInternal() {
    ...
    // 初始化WebSocket连接（最后执行，确保其他初始化已完成）
    initializeWebSocket();
}
...
function initializeWebSocket() {
    ...
    socket.on('message-deleted', (data) => {
        ...
        if (messageId) {
            ...
            // 如果有chatHistory，删除其中对应的消息HTML结构
            if (chatHistory) {
                if (chatHistory.nodeType === 11) { // 11 是文档片段的节点类型
                    // 遍历文档片段中的所有消息元素，找到并删除匹配的消息
    ...
    socket.on('private-message-received', (message) => {
        ...
        if (currentActiveChat === `private_${chatPartnerId}`) {
            moveFriendToTop(chatPartnerId);
        }
...
function displayMessage(message, returnElement = false) {
    ...
    // 如果没有messageContainer但是有chatHistory，继续执行
    ...
    if (messageContainer) {
        ...
        // 如果没有messageContainer但是有chatHistory，将消息添加到chatHistory
        if (chatHistory.nodeType === 11) { // 11 是文档片段的节点类型
...
function displayGroupMessage(message, returnElement = false) {
    ...
    // 如果没有groupMessageContainer但是有chatHistory，继续执行
...
function handleSettingsSubmit(form) {
    ...
    } else if (settingId === 'change-signature') {
        // 获取新个性签名
        const newSignature = document.getElementById('newSignature').value;

        // 发送个性签名修改请求到后端
        fetch(`${SERVER_URL}/update-signature`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'session-token': currentSessionToken,
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    showError(data.message || '个性签名修改失败');
                }
            })
            .catch(error => {
                console.error('个性签名修改失败:', error);
                showError('个性签名修改失败');
            });
...
export function initializeScrollLoading() {
    ...
    function handleScroll(e, container, isGroup, isPrivate = false) {
        ...
        if (container.scrollTop < 50) { // 使用50px的阈值，避免必须滚动到绝对顶部
            // console.log('📜 [滚动加载] 触发向上滚动加载条件，scrollTop:', container.scrollTop);
            // 避免频繁触发
            if (!window.isLoadingMoreMessages) {
                // console.log('📜 [滚动加载] 开始加载更多消息');
                window.isLoadingMoreMessages = true;

                // 记录当前滚动位置信息（用于加载后恢复）
                const prevScrollHeight = container.scrollHeight;
                const prevScrollTop = container.scrollTop;
                // console.log('📜 [滚动加载] 记录滚动位置 - scrollHeight:', prevScrollHeight, 'scrollTop:', prevScrollTop);
                ...
                if (messages.length > 0) {
                    ...
                    // console.log('📜 [滚动加载] 获取最早消息sequence:', olderThan);
                } else {
                    // console.log('📜 [滚动加载] 没有找到消息元素');
                }

                // 发送加载更多请求
                if (currentUser && currentSessionToken) {
                    // console.log('📜 [滚动加载] 用户已登录，发送加载请求');
                    if (isGroup && currentGroupId) {
                        // console.log('📜 [滚动加载] 加载群组消息，groupId:', currentGroupId);
                        // 加载群组消息
                        const joinGroupData = {
                            groupId: currentGroupId,
                            sessionToken: currentSessionToken,
                        ...
                        // console.log('📜 [滚动加载] 加载私信消息，userId:', currentPrivateChatUserId);
                        // 加载私信消息
                        const historyOptions = {
                            userId: currentUser.id,
                            friendId: currentPrivateChatUserId,
                    ...
                    // console.log('📜 [滚动加载] 用户未登录，取消加载');
                    window.isLoadingMoreMessages = false;
                }
            } else {
                // console.log('📜 [滚动加载] 正在加载中，跳过重复触发');
            }
        }
    }
...
// 启动初始化 - 已移至 initializeChat 函数中调用
...
export function setupEventDelegation() {
    ...
    function handleAvatarClick(e) {
        ...
        if (userId) {
            // 尝试从onlineUsersList或friendsList中获取完整的用户信息
            let fullUser = null;
            
            // 先从onlineUsersList中查找
            ...
            // 如果找到完整用户信息，传递给弹窗
            if (fullUser) {
                showUserAvatarPopup(e, fullUser);
            } else {
                // 否则只传递基本信息
                showUserAvatarPopup(e, { id: userId, nickname: nickname });
            }
    ...
    // 离线用户列表容器
    const offlineUserListElement = document.getElementById('offlineUserList');
    if (offlineUserListElement) {
        offlineUserListElement.addEventListener('click', function(e) {
            // 头像点击 - 检查点击的是否是头像或包含头像的元素
            const avatarElement = e.target.closest('.user-avatar');
            if (avatarElement) {
                e.stopPropagation();
    ...
    if (friendsListElement) {
        friendsListElement.addEventListener('click', function(e) {
            // 头像点击 - 检查点击的是否是头像或包含头像的元素
            const avatarElement = e.target.closest('.user-avatar');
            if (avatarElement) {
                e.stopPropagation();
...
export function login() {
    ...
    // 确保currentUser和currentSessionToken已设置
...
export function saveChatHistory(type) {
    ...
    if (type === 'save') {
        ...
        // 通过appendChild添加克隆的子元素到消息容器
        if (chatHistory) {
            // 如果chatHistory是文档片段，直接appendChild
            if (chatHistory.nodeType === 11) { // 11 是文档片段的节点类型
